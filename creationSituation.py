...
# Cette fonction sert à vérifier si deux avions sont en conflit
...
def cercle_deforme(m, RAYON_CERCLE, liste_vitesse_avion, epsilon):
    ...
    liste_position = [
        np.array([RAYON_CERCLE * np.cos(m * k + deformation), RAYON_CERCLE * np.sin(m * k + deformation)])]
    ...
    while nombre < ct.N_avion:
        ...
        if initialConflict(liste_position, val):  # s'il n'y a pas de conflit
    ...
    for pos in liste_position:
        if pos[0] >= 0:  # Valeur de la coordonnée x de l'avion
            ...
            if pos[1] >= 0:
                ...
                liste_angle.append(angle_hasard)
    return [pb.Flight(liste_vitesse_avion[k], QPoint(liste_position[k][0], liste_position[k][1]), liste_angle[k],
                      pb.Manoeuvre(ct.T, 0, 0)) for k in range(ct.N_avion)]
